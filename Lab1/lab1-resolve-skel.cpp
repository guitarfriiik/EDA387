...
int main( int aArgc, char* aArgv[] )
{
	...
	printf( "Resolving `%s' from `%s':\n", remoteHostName, localHostName );
 	...
 	struct addrinfo *r_addrInfo; // Result info..
	...
	if((error = getaddrinfo(remoteHostName, NULL, &hints, &r_addrInfo)) < 0){
	  printf("%s \n", gai_strerror(error));
	  return 1;
	}
	
	struct addrinfo *addrItr = r_addrInfo;
	char tempBuf[100];
	
	while(addrItr != NULL){
	  inet_ntop(AF_INET,(void *) &((sockaddr_in *)(addrItr->ai_addr))->sin_addr.s_addr, tempBuf, sizeof(tempBuf));
	  printf("%s has adress %s \n", remoteHostName, tempBuf);
	  addrItr = addrItr->ai_next;
	}
	
	freeaddrinfo(r_addrInfo);
